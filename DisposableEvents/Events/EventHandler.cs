...
public sealed class EventHandler<TState, TMessage> : IEventHandler<TMessage> {
    readonly Action<TState, TMessage> handler;
    readonly TState state;

    public EventHandler(TState state, Action<TState, TMessage> handler) {
        this.state = state;
...
public sealed class VoidEventHandler<TState> : IEventHandler<Void> {
    readonly Action<TState> handler;
    readonly TState state;

    public VoidEventHandler(TState state, Action<TState> handler) {
        this.state = state;
        this.handler = handler;
    }
