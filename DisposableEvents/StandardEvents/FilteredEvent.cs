...
public sealed class FilteredEvent<TMessage> : IPipelineEvent<TMessage> {
    ...
    public FilteredEvent(ReadOnlySpan<IEventFilter<TMessage>> filters, int initialSubscriberCapacity = -1)
        : this(CompositeEventFilter<TMessage>.Create(filters), initialSubscriberCapacity) { }
    public FilteredEvent(ReadOnlySpan<IEventFilter<TMessage>> filters, FilterOrdering ordering, int initialSubscriberCapacity = -1) 
...
public static class EventPipelineFilterExtensions {
    public static EventPipelineBuilder<TMessage> Filter<TMessage>(this EventPipelineBuilder<TMessage> builder, ReadOnlySpan<IEventFilter<TMessage>> filters, FilterOrdering ordering = FilterOrdering.StableSort) {
    ...
    public static EventPipelineBuilder<TMessage> Filter<TState, TMessage>(this EventPipelineBuilder<TMessage> builder, TState state, Func<TState, TMessage, bool> predicateFilter) {
        var preFilteredEvent = new FilteredEvent<TMessage>(new StatefulPredicateEventFilter<TState, TMessage>(state, predicateFilter));
        return builder.Next(preFilteredEvent);
    }
}