...
public interface IEmptyEventFilter : IEventFilter {
    bool Filter();
}
...
public interface IEventFilter<T> : IEventFilter { 
    /// <summary>
...
public class PredicateEventFilter<T> : IEventFilter<T> {
    ...
    public PredicateEventFilter(int filterOrder, Func<T, bool> predicate) {
        FilterOrder = filterOrder;
        this.predicate = predicate;
    }

    public bool Filter(ref T value) => predicate?.Invoke(value) ?? true;
}
...
public class CompositeEventFilter<TMessage> : IEventFilter<TMessage> {
    ...
    public bool Filter(ref TMessage value) {
        foreach (var filter in orderedFilters) {
            if (!filter.Filter(ref value))
                return false;
        }
        return true;
    }
