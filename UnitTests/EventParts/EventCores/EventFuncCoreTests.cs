...
[TestFixture]
public class EventFuncCoreTests {
    ...
    public void Observer_OnNextThrows_CallsOnErrorAndContinues() {
        ...
        var throwingObs = new ThrowingFuncObserver<int, int>(error);
    ...
    public void Publish_NullMessage_DoesNotThrow() {
        var core = new EventFuncCore<string, int>();
        var obs = new TestFuncObserver<string, int>();
    ...
    public void PubSub_Handles1000Subscribers() {
        var core = new EventFuncCore<int, int>();
        var observers = new List<TestFuncObserver<int, int>>();
    ...
    public void PubSub_Handles1000SubscribersWithErrors() {
        var core = new EventFuncCore<int, int>();
        var observers = new List<IEventFuncObserver<int, int>>();
        var error = new Exception("Test error");

        for (int i = 0; i < 1000; i++) {
            IEventFuncObserver<int, int> obs =
                i % 2 == 0 ? new TestFuncObserver<int, int>() : new ThrowingFuncObserver<int, int>(error);
            observers.Add(obs);
            core.Subscribe(obs);
        }
    ...
    public void Publish_ReturnsLastSuccess() {
        ...
        var obs2 = new TestFuncObserver<int, int>(10, false);
    ...
    public void PublishEnumerator_None() {
        ...
        var obs1 = new TestFuncObserver<int, int>(5, true);
        var obs2 = new TestFuncObserver<int, int>(10, false);
        var throwingObs = new ThrowingFuncObserver<int, int>(new Exception("Test error"));
        ...
        Assert.Multiple(() => {
            ...
            Assert.That(results[0].IsSuccess, Is.True, "First observer should return success");
            ...
            Assert.That(results[2].IsSuccess, Is.False, "Throwing observer should return failure");
        });
    }
